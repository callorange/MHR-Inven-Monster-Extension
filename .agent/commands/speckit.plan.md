---
description: 구현 계획 워크플로우를 실행하고 plan 템플릿을 사용하여 설계 산출물을 생성합니다.
handoffs: 
  - label: 태스크 생성 (Create Tasks)
    agent: speckit.tasks
    prompt: 이 계획(plan)을 태스크 목록으로 나누어 주세요
    send: true
  - label: 체크리스트 생성 (Create Checklist)
    agent: speckit.checklist
    prompt: 다음 도메인에 대한 체크리스트를 생성해 주세요...
---
