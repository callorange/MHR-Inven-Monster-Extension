...
function Extract-PlanField {
    ...
    # FieldPattern may contain '|' for translations, so escape the parts separately
    $escapedParts = $FieldPattern -split '\|' | ForEach-Object { [Regex]::Escape($_) }
    $escapedPattern = $escapedParts -join '|'
    $regex = "^\*\*(?:$escapedPattern)\*\*: (.+)$"
    Get-Content -LiteralPath $PlanFile -Encoding utf8 | ForEach-Object {
        if ($_ -match $regex) { 
            $val = $Matches[1].Trim()
            if ($val -notin @('NEEDS CLARIFICATION','N/A','명확화 필요','해당 없음')) { return $val }
        }
...
function Parse-PlanData {
    ...
    $script:NEW_LANG        = Extract-PlanField -FieldPattern 'Language/Version|언어/버전' -PlanFile $PlanFile
    $script:NEW_FRAMEWORK   = Extract-PlanField -FieldPattern 'Primary Dependencies|주요 의존성' -PlanFile $PlanFile
    $script:NEW_DB          = Extract-PlanField -FieldPattern 'Storage|저장소' -PlanFile $PlanFile
    $script:NEW_PROJECT_TYPE = Extract-PlanField -FieldPattern 'Project Type|프로젝트 유형' -PlanFile $PlanFile
...
function Format-TechnologyStack {
    ...
    $parts = @()
    if ($Lang -and $Lang -notin @('NEEDS CLARIFICATION','명확화 필요')) { $parts += $Lang }
    if ($Framework -and $Framework -notin @('NEEDS CLARIFICATION','N/A','명확화 필요','해당 없음')) { $parts += $Framework }
    if (-not $parts) { return '' }
    return ($parts -join ' + ')
}
...
function New-AgentFile {
    ...
    if ($escaped_lang -and $escaped_framework) {
        $recentChangesForTemplate = "- ${escaped_branch}: 추가됨 - ${escaped_lang} + ${escaped_framework}"
    } elseif ($escaped_lang) {
        $recentChangesForTemplate = "- ${escaped_branch}: 추가됨 - ${escaped_lang}"
    } elseif ($escaped_framework) {
        $recentChangesForTemplate = "- ${escaped_branch}: 추가됨 - ${escaped_framework}"
    }
...
function Update-ExistingAgentFile {
    ...
    if ($NEW_DB -and $NEW_DB -notin @('N/A','NEEDS CLARIFICATION','해당 없음','명확화 필요')) {
    ...
    $newChangeEntry = ''
    if ($techStack) { $newChangeEntry = "- ${CURRENT_BRANCH}: 추가됨 - ${techStack}" }
    elseif ($NEW_DB -and $NEW_DB -notin @('N/A','NEEDS CLARIFICATION','해당 없음','명확화 필요')) { $newChangeEntry = "- ${CURRENT_BRANCH}: 추가됨 - ${NEW_DB}" }
    ...
    for ($i=0; $i -lt $lines.Count; $i++) {
        $line = $lines[$i]
        if ($line -match '^##\s*(Active Technologies|활성 기술)\s*$') {
            $output.Add($line)
            $inTech = $true
            continue
        }
        ...
        if ($line -match '^##\s*(Recent Changes|최근 변경 사항)\s*$') {
            $output.Add($line)
        ...
        if ($line -match '(\*\*)?(Last updated|마지막 업데이트)(\*\*)?: .*\d{4}-\d{2}-\d{2}') {
